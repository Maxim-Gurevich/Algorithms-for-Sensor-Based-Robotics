...
while (norm(omega_b)>eps_omega || norm(v_b) > eps_v) && i<100
    ...
    scale=.5;
    for z = 1:3
        ellipsoid_x(end+1) = vec_dir(1,z)*l(z)*scale;
        ellipsoid_y(end+1) = vec_dir(2,z)*l(z)*scale;
        ellipsoid_z(end+1) = vec_dir(3,z)*l(z)*scale;
        
        ellipsoid_x(end+1) = vec_dir(1,z)*-1*l(z)*scale;
        ellipsoid_y(end+1) = vec_dir(2,z)*-1*l(z)*scale;
        ellipsoid_z(end+1) = vec_dir(3,z)*-1*l(z)*scale;
    end
